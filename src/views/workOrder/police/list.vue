...
<!--            <el-button class="filter-item" type="primary" @click="handleView">详情</el-button>-->
            <i class="iconfont icon-xiangqing baseColor inlineBlock" @click.stop="handleView"></i>
            <i class="iconfont icon-daochufffpx baseColor inlineBlock ml_10" @click.stop="handleView"></i>
          </template>
        </el-table-column>
      </el-table>

      <ul class="img_list flex" style="height:537px;" v-if="displayType=='imgList'">
        <li v-for="(item,index) in list" :key="index" @click="handleView">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleView(row){
        this.$router.push({path:'police/view',query: {id:'1'}})
